...
import { validatePng } from "../../dist/artifacts.js";
...
try {
  ...
  for (const name of ["hero", "editorial", "continuation"]) {
    const bytes = await readFile(values[name]);
  ...
  const text = (slide, value, x, y, w, h, options = {}) => slide.addText(value, {
    x, y, w, h, margin: 0, breakLine: false, fontFace: "Trebuchet MS",
  ...
  text(slide, "Same story.\nA new light.", 0.65, 1.55, 3.95, 2.0, { fontFace: "Georgia", fontSize: 43 });
  text(slide, "Choose the source explicitly.\nDescribe what should change.", 0.65, 4.2, 3.95, 0.8, { fontSize: 16 });
  text(slide, "Source-aware, not pixel-identical.", 0.65, 6.75, 4.05, 0.3, { fontSize: 10, color: "CCD6DD" });
