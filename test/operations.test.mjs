...
test("replays are free, changed args conflict, uncertain outcomes never resubmit", async () => {
  ...
  try {
    ...
    await assert.rejects(store.run(uncertain, {}, async () => { calls++; throw new Error("timeout"); }), /timeout/);
    await assert.rejects(restarted.run(uncertain, {}, submit), { code: "outcome_unknown" });
